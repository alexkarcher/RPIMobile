...
//    [self.navigationController.toolbar setTintColor:[UIColor colorWithHex:0xC20000]];
...
- (void) swapViews {
    STEtasViewController *_etaView = [[STEtasViewController alloc] initWithStyle:UITableViewStyleGrouped];
    [self.navigationController pushViewController:_etaView animated:YES];
}
...
- (void)viewDidLoad {
    ...
    RMAppDelegate *_delegate = (RMAppDelegate *)[[UIApplication sharedApplication] delegate];
    self.dataManager = _delegate.dataManager;
    self.managedObjectContext = _delegate.managedObjectContext;
